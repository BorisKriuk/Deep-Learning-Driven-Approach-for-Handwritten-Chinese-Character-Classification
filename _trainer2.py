...
# Define a custom preprocessing function for blurring images
def apply_blur(img):
    blurred_image=cv2.GaussianBlur(img, (3, 3), 20)
    return blurred_image[:, :, np.newaxis]
...
if len(x.shape) == 3:  # Shape should be (batch_size, height, width)
...
def show_grid(image_list, nrows, ncols, label_list=None, show_labels=False, figsize=(10,10)):
    ...
    for i in range(nrows * ncols):
        ...
        image = image_list[i]
        if image.ndim == 2:  # Check if the image is 2D
            image = image[:, :, np.newaxis]  # Add a new axis to make it 3D
        ax.imshow(image.squeeze(), cmap='Greys_r')  # Use squeeze() to remove singleton dimensions
        ax.axis('off')

# Your code to get the images from the generator and show the grid
x, y = next(original_train_gen)
